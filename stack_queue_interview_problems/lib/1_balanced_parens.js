...
function balancedParens(str) {
    const num = {paren: 0, bracket: 0, curly: 0};
    let currentParen = "";
    for(let i = 0; i < str.length; i++){
        ...
        if(char === "(") {
            ...
            if(currentParen[currentParen.length - 1] !== "(") return false;
            currentParen = currentParen.slice(0, -1);
            num.paren--;
        }
        else if(char === "["){ 
            num.bracket++;
        ...
        if(num.paren < 0 || num.bracket < 0 || num.curly < 0) return false;
    }
    return (num.paren === 0 && num.bracket === 0 && num.curly === 0);
}
